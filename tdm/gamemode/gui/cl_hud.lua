...
function BF4HUD( )
	...
	if IsValid(BFply) and BFply:Alive() and IsValid(BFwep) then
		...
		local RoundTimeS = tonumber(BFply:GetNWInt("RoundTimeLeftS"))
		local RoundTimeM = tonumber(BFply:GetNWInt("RoundTimeLeftM"))
		
		if RoundTimeS < 10 then
			draw.SimpleText(RoundTimeM..":0"..RoundTimeS, "BF4_Counters", ScrW()/2 - 35, 75, Color(158,167,171,330), 0, 0)
		else
			draw.SimpleText(RoundTimeM..":"..RoundTimeS, "BF4_Counters", ScrW()/2 - 35, 75, Color(158,167,171,330), 0, 0)
		end
		...
		for k,v in pairs(ents.FindInSphere(BFply:GetPos(),2048)) do
			...
			if v:IsNPC() or v:IsPlayer() and v != BFply then
				...
				if v:IsNPC() or v:Team() != BFply:Team() then
