...
local function inventoryItemButton( iname, name, amount, desc, model, slot, parent, dist, buttons )
	...
	local function ItemButtons()
		for k,v in pairs(inventory) do
			...
			if i then
				local buttons = {}
				PlayerLoadoutTable = {}
				buttons["Add To Loadout"] = function()
					inventoryUse(k)
					f:Close()
					end
					
				local b = inventoryItemButton( k, i.name .. "(" .. v.amount .. ")", v.amount, i.desc, i.model, i.slot, items, i.buttonDist, buttons )
				items:AddItem(b)
			end
		end
	end
